...
func TestAponoBundleV2Resource(t *testing.T) {
	...
	t.Run("ReadNotFound", func(t *testing.T) {
		...
		notFoundErr := &client.NotFoundError{} // Use the custom NotFoundError defined above
		mockInvoker.EXPECT().
			GetBundleV2(mock.Anything, mock.Anything).
	...
	t.Run("PlanValidation_ExclusivityViolation_MultipleKindsSet", func(t *testing.T) {
		...
		// Introduce conflict: AccessTargets[0] already has Integration set; add QueryTarget too
	...
	t.Run("PlanValidation_ExclusivityViolation_NoKindSet", func(t *testing.T) {
		...
		model.AccessTargets[0] = models.BundleAccessTargetModel{}
