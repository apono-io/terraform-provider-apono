/*
Apono

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package aponoapi

import (
	"encoding/json"
	"fmt"
)

// checks if the TagTerraformV1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TagTerraformV1{}

// TagTerraformV1 struct for TagTerraformV1
type TagTerraformV1 struct {
	Name  string `json:"name"`
	Value string `json:"value"`
}

type _TagTerraformV1 TagTerraformV1

// NewTagTerraformV1 instantiates a new TagTerraformV1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTagTerraformV1(name string, value string) *TagTerraformV1 {
	this := TagTerraformV1{}
	this.Name = name
	this.Value = value
	return &this
}

// NewTagTerraformV1WithDefaults instantiates a new TagTerraformV1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTagTerraformV1WithDefaults() *TagTerraformV1 {
	this := TagTerraformV1{}
	return &this
}

// GetName returns the Name field value
func (o *TagTerraformV1) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *TagTerraformV1) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *TagTerraformV1) SetName(v string) {
	o.Name = v
}

// GetValue returns the Value field value
func (o *TagTerraformV1) GetValue() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Value
}

// GetValueOk returns a tuple with the Value field value
// and a boolean to check if the value has been set.
func (o *TagTerraformV1) GetValueOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Value, true
}

// SetValue sets field value
func (o *TagTerraformV1) SetValue(v string) {
	o.Value = v
}

func (o TagTerraformV1) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TagTerraformV1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["name"] = o.Name
	toSerialize["value"] = o.Value
	return toSerialize, nil
}

func (o *TagTerraformV1) UnmarshalJSON(bytes []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"name",
		"value",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(bytes, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varTagTerraformV1 := _TagTerraformV1{}

	err = json.Unmarshal(bytes, &varTagTerraformV1)

	if err != nil {
		return err
	}

	*o = TagTerraformV1(varTagTerraformV1)

	return err
}

type NullableTagTerraformV1 struct {
	value *TagTerraformV1
	isSet bool
}

func (v NullableTagTerraformV1) Get() *TagTerraformV1 {
	return v.value
}

func (v *NullableTagTerraformV1) Set(val *TagTerraformV1) {
	v.value = val
	v.isSet = true
}

func (v NullableTagTerraformV1) IsSet() bool {
	return v.isSet
}

func (v *NullableTagTerraformV1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTagTerraformV1(val *TagTerraformV1) *NullableTagTerraformV1 {
	return &NullableTagTerraformV1{value: val, isSet: true}
}

func (v NullableTagTerraformV1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTagTerraformV1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
