...
### Query Target — Single Inline Query
...
### Query Target — Heredoc Form to Avoid Escaping Double Quotes

HCL has no single-quoted string, so any literal `"` inside a `"..."` query has to be escaped as `\"`. For readability, prefer a heredoc (`<<-EOT ... EOT`) — the AQL goes in verbatim, no escapes:
...
Equivalent to the escaped form `"integration in (\"aws-rds\") and resource_tag[\"environment\"] = \"production\""` but easier to read and review.

### Query Target Mixed with Other Target Kinds
...
### Query Target — Multi-line AQL via Heredoc
...
- `query_target` (Attributes) Query target. Resources matched by an inline AQL expression evaluated at access-resolution time. (see [below for nested schema](#nestedatt--access_targets--query_target))
...
<a id="nestedatt--access_targets--query_target"></a>
### Nested Schema for `access_targets.query_target`
